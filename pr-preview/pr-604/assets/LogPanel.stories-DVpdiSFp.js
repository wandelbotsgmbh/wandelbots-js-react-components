import{i as e,s as t}from"./preload-helper-Cs4UwXAW.js";import{Ga as n,Jt as r,L as i,Qa as a,ai as o,oi as s,qt as c,xt as l}from"./iframe-BvHxi_cZ.js";var u,d,f,p,m,h,g,_;e((()=>{o(),c(),u=t(a(),1),i(),d=n(),f={title:`Components/LogPanel`,component:l,tags:[`!dev`],parameters:{docs:{description:{component:`A complete log panel component with built-in state management using MobX. 
...
\`\`\``}}},argTypes:{store:{control:!1,description:`Optional LogStore instance. If not provided, creates its own.`},height:{control:{type:`text`},description:`Height of the component`,defaultValue:400},onStoreReady:{action:`store-ready`,description:`Callback when the store is ready for external access`}}},p={render:e=>{let[t,n]=(0,u.useState)(null),i=t=>{n(t),e.onStoreReady?.(t)};return(0,d.jsxs)(r,{spacing:2,sx:{width:600},children:[(0,d.jsxs)(r,{direction:`row`,spacing:1,children:[(0,d.jsx)(s,{variant:`outlined`,size:`small`,onClick:()=>{t?.addInfo(`Info: Operation completed at ${new Date().toLocaleTimeString()}`)},disabled:!t,children:`Add Info`}),(0,d.jsx)(s,{variant:`outlined`,size:`small`,onClick:()=>{t?.addWarning(`Warning: System approaching limits at ${new Date().toLocaleTimeString()}`)},color:`warning`,disabled:!t,children:`Add Warning`}),(0,d.jsx)(s,{variant:`outlined`,size:`small`,onClick:()=>{t?.addError(`Error: Connection failed at ${new Date().toLocaleTimeString()}`)},color:`error`,disabled:!t,children:`Add Error`}),(0,d.jsx)(s,{variant:`outlined`,size:`small`,onClick:()=>{t?.addDebug(`Debug: Function called with parameters {id: ${Math.floor(Math.random()*1e3)}, timestamp: ${new Date().toISOString()}}`)},disabled:!t,sx:{color:`text.disabled`,borderColor:`text.disabled`},children:`Add Debug`})]}),(0,d.jsx)(l,{...e,onStoreReady:i})]})},args:{height:400},parameters:{docs:{description:{story:`Interactive log panel with built-in state management. Use the buttons above to add different types of log messages. The component manages its own LogStore internally.`}}}},m={render:e=>{let t=e=>{e.addInfo(`Nova client initialized and connected to robot controller`),e.addDebug(`Connection parameters: {host: '192.168.1.100', port: 8080, timeout: 5000}`),e.addWarning(`Joint 3 approaching velocity limit - reducing speed to 80%`),e.addError(`Safety zone violation detected - emergency stop triggered`),e.addDebug(`Error context: {zone_id: 'SZ_001', tcp_position: [245.67, -123.45, 378.90]}`)};return(0,d.jsx)(l,{...e,onStoreReady:t})},args:{},parameters:{docs:{description:{story:`LogPanel with default height (400px) and some sample messages.`}}}},h={render:e=>{let[t,n]=(0,u.useState)(null),i=e=>{n(e);for(let t=1;t<=50;t++){let n=t%10==0?`debug`:t%7==0?`error`:t%3==0?`warning`:`info`,r=new Date(Date.now()-(50-t)*1e3);n===`debug`?e.addDebug(`Debug trace #${t}: Function robotController.getJointPositions() returned [${t*15%180}, ${t*20%180}, ${t*25%180}, ${t*30%180}, ${t*35%180}, ${t*40%180}] in ${t%10+1}ms`):n===`error`?e.addError(`Robot motion error #${t}: Joint ${t%6+1} position feedback timeout. Communication with servo drive lost. Check encoder cables and power supply connections. Recovery requires manual joint reset procedure.`):n===`warning`?e.addWarning(`Motion planning warning #${t}: Path optimization detected potential singularity near joint configuration J${t%6}=${t*15%180}°`):e.addInfo(`Motion sequence #${t}: Waypoint reached successfully at ${r.toLocaleTimeString()}, TCP velocity: ${t*50%1e3}mm/s`)}};return(0,d.jsxs)(r,{spacing:2,sx:{width:700},children:[(0,d.jsx)(r,{direction:`row`,spacing:1,children:(0,d.jsx)(s,{variant:`outlined`,size:`small`,onClick:()=>{if(!t)return;let e=[`Nova client connected to robot controller at 192.168.1.100`,`Motion group 0@ur5e initialized successfully`,`Wandelscript program compiled and validated`,`Robot moved to home position [0.0, -90.0, 0.0, -90.0, 0.0, 0.0]`,`Safety zone collision detected - motion stopped`,`TCP calibration completed with accuracy ±0.1mm`,`Joint velocity limits updated: [180°/s, 180°/s, 180°/s, 360°/s, 360°/s, 360°/s]`,`Emergency stop button pressed - all motions halted`,`Robot workspace boundaries recalibrated`,`Nova API rate limit: 100 requests remaining in current window`,`Gripper IO signal received: digital_out_0 = HIGH`,`Path planning optimization reduced cycle time by 12%`,`Robot temperature monitoring: Joint 1 at 45°C (normal range)`,`Wandelbots skill library updated to version 2.1.3`,`Motion execution completed in 4.2 seconds`,`Safety controller heartbeat lost - entering safe mode`,`Robot pose validation failed: position outside workspace`,`Nova WebSocket connection established with latency 15ms`,`Coordinate system transformation applied: base_link → tool0`,`Program execution paused - awaiting operator confirmation`],n=[`info`,`warning`,`error`,`debug`],r=e[Math.floor(Math.random()*e.length)],i=n[Math.floor(Math.random()*n.length)];t.addMessage(r,i)},disabled:!t,children:`Add Random Message`})}),(0,d.jsx)(l,{...e,onStoreReady:i})]})},args:{height:300},parameters:{docs:{description:{story:`LogPanel with many messages to demonstrate scrolling behavior. The component automatically scrolls to show the latest messages, and long messages are properly wrapped.`}}}},g={render:e=>{let[t,n]=(0,u.useState)(null),i=e=>{n(e),e.addInfo(`Simple connection established`),e.addWarning(`Motion planning detected potential collision with safety zone SZ_001. Recommended path deviation: 15mm offset along Y-axis to maintain 50mm safety margin.`),e.addError(`Fatal robot communication error occurred at ${new Date().toISOString()}: 
...
Reference: ERR_COMM_2024_0819_001`),e.addInfo(`Recovery procedure initiated`),e.addError(`Wandelscript compilation failed with multiple syntax errors:
...
For detailed syntax reference, see: https://docs.wandelbots.com/wandelscript/syntax`)};return(0,d.jsxs)(r,{spacing:2,sx:{width:800},children:[(0,d.jsx)(r,{direction:`row`,spacing:1,children:(0,d.jsx)(s,{variant:`outlined`,size:`small`,onClick:()=>{if(!t)return;let e=[`Stack trace for robot motion exception:
...
Recommendations: No issues detected. System operating optimally.`],n=e[Math.floor(Math.random()*e.length)];t.addError(n)},disabled:!t,children:`Add Long Message`})}),(0,d.jsx)(l,{...e,onStoreReady:i})]})},args:{height:400},parameters:{docs:{description:{story:`LogPanel demonstrating very long log messages with detailed stack traces, error reports, and system diagnostics. Long messages are automatically wrapped and can be scrolled within the log viewer.`}}}},p.parameters={...p.parameters,docs:{...p.parameters?.docs,source:{originalSource:`{
  render: args => {
...
}`,...g.parameters?.docs?.source}}},_=[`Interactive`,`DefaultHeight`,`ScrollingExample`,`LongMessagesExample`]}))();export{m as DefaultHeight,p as Interactive,g as LongMessagesExample,h as ScrollingExample,_ as __namedExportsOrder,f as default};