...
The theme currently supports **dark mode only**. Passing `palette: { mode: "light" }` is silently ignored (with a console warning) — light tokens are not yet available from Figma.
