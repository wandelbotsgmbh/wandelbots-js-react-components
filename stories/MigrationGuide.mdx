import { Meta } from '@storybook/addon-docs/blocks';

<Meta title="Migration Guide" />

# Migration Guide

This guide helps you migrate to the latest version of the Wandelbots React Components library.

## Breaking Changes from 2.x (API v1) to 3.x (API v2)

### ConnectedMotionGroup

- **Import from this library, not from nova-js**: `ConnectedMotionGroup` must now be imported from `@wandelbots/wandelbots-js-react-components/core` instead of `@wandelbots/nova-js`
- **Initialization**: Use the static `connect` method with a `Nova` instance and motion group ID

```tsx
// ❌ Old (don't use)
import { ConnectedMotionGroup } from "@wandelbots/nova-js"

// ✅ New (correct)
import { Nova } from "@wandelbots/nova-js/v2"
import { ConnectedMotionGroup } from "@wandelbots/wandelbots-js-react-components/core"

const nova = new Nova({ instanceUrl: "https://your-nova-instance.com" })
const connectedMotionGroup = await ConnectedMotionGroup.connect(nova, "0@your-motion-group-id")
```

### Nova.js V2 API

- **All components use V2**: All components now use the Nova.js V2 API (`@wandelbots/nova-js/v2`)

```tsx
// ✅ For all components, use V2
import { NovaClient } from "@wandelbots/nova-js/v2"
```

### Component Props

- **Nova or string**: Many components now accept either a `Nova` instance or a URL string for the `nova` prop
- **connectedMotionGroup prop**: Components that work with robots now expect a `ConnectedMotionGroup` instance

```tsx
// ✅ Option 1: Pass Nova instance
<JoggingPanel nova={nova} motionGroupId="0@ur5e" />

// ✅ Option 2: Pass URL string
<JoggingPanel nova="https://your-nova-instance.com" motionGroupId="0@ur5e" />

// ✅ Where applicable, pass ConnectedMotionGroup
<RobotCard connectedMotionGroup={connectedMotionGroup} />
```

## Migration Steps

### Update Imports

Replace all imports of `ConnectedMotionGroup` from `@wandelbots/nova-js` to this library:

```tsx
// Before
import { ConnectedMotionGroup } from "@wandelbots/nova-js"

// After
import { ConnectedMotionGroup } from "@wandelbots/wandelbots-js-react-components/core"
```

### Update NovaClient Usage

Switch to the V2 API for NovaClient:

```tsx
// Before
import { NovaClient } from "@wandelbots/nova-js"

// After
import { NovaClient } from "@wandelbots/nova-js/v2"
```

### Update ConnectedMotionGroup Initialization

Use the new `connect` method:

```tsx
// Before
const connectedMotionGroup = new ConnectedMotionGroup(nova, motionGroupId)

// After
const connectedMotionGroup = await ConnectedMotionGroup.connect(nova, motionGroupId)
```

### Update Component Usage

Ensure components receive the correct props:

```tsx
// Robot rendering components
import { Nova } from "@wandelbots/nova-js/v2"
import { ConnectedMotionGroup } from "@wandelbots/wandelbots-js-react-components/core"
import { RobotCard, Robot } from "@wandelbots/wandelbots-js-react-components/3d"

const nova = new Nova({ instanceUrl: "https://your-instance.com" })
const connectedMotionGroup = await ConnectedMotionGroup.connect(nova, "0@ur5e")

// Use with 3D components
<RobotCard connectedMotionGroup={connectedMotionGroup} />
<Robot connectedMotionGroup={connectedMotionGroup} />

// Jogging components
<JoggingPanel nova={nova} motionGroupId="0@ur5e" />
```

## Breaking Changes from 5.x to 6.x (`Nova` replaces `NovaClient`)

This library now uses the new `Nova` client from `@wandelbots/nova-js/v2`. The deprecated `NovaClient` baked in a cell id and exposed cell-scoped API methods; `Nova` is cell-agnostic and every cell-scoped API method now takes the cell id as its first argument.

Components and helpers that previously accepted a `NovaClient` now accept a `Nova` instance (or a URL string) and an optional `cellId` (defaulting to `"cell"`).

```tsx
// Before
import { NovaClient } from "@wandelbots/nova-js/v2"
const nova = new NovaClient({ instanceUrl: "https://your-nova-instance.com" })
<JoggingPanel nova={nova} motionGroupId="0@ur5e" />

// After
import { Nova } from "@wandelbots/nova-js/v2"
const nova = new Nova({ instanceUrl: "https://your-nova-instance.com" })
<JoggingPanel nova={nova} motionGroupId="0@ur5e" cellId="cell" />
```

Affected helpers (`cellId` accepted as an option):
- `ConnectedMotionGroup.connect(nova, motionGroupId, { cellId })`
- `ConnectedMotionGroup.connectMultiple(nova, motionGroupIds, { cellId })`
- `MotionStreamConnection.open(nova, motionGroupId, { cellId })`
- `JoggerConnection.open(nova, motionGroupId, { cellId, ... })`

## Breaking Changes from 4.x (API V1 & V2 both supported) to 5.x (API V2 only)

### No support for V1 data
- The components of this package do not accept data coming from V1 API anymore. Before using the package version from 5, ensure your application is currently running on V2 API.
- Components, that were still supporting V1 data: `CoordinateSystemTransform`, `SafetyZoneRenderer`, `TrajectoryRenderer`, `CollisionSceneRenderer`

### Wandelscript Editor
- After its deprecation, Wandelscript and all corresponding components / functions, eg. `WandelScriptEditor` are removed. Make sure the consumer application does not include mentioned functionalities or components anymore.

## Common Issues

### "Module not found" errors

- Ensure you're importing from the correct sub-paths: `/3d` or `/core`
- Check that peer dependencies are installed: `@wandelbots/nova-js`, `@mui/material`, `react`, `react-dom`

### Components not receiving motion state updates

- Verify that your `ConnectedMotionGroup` is properly connected before passing it to components
