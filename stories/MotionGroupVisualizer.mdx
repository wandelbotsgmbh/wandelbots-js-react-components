...
The differentiation is dependent on the `inverseSolver` and `dhParameters` variables, which must be passed as a prop:
- `dhParameters` have to include the `type` attribute, that determines, which joint type is supported by the deliver model
