...
export const ProgramControl = externalizeComponent(
  observer(({
    ...
    const getButtonIcon = (index: number) => {
      ...
      if (variant === 'without_pause') {
        return index === 0 ? <PlayArrow {...iconProps} /> : <Stop {...iconProps} />;
      }
        ...
        >
          {buttonConfigs.map((config, index) => (
            ...
            >
              <Button
                variant="contained"
                disabled={!config.enabled || (state === 'stopping' && !requiresManualReset)}
                ...
                sx={{
                  ...
                  opacity: config.enabled && !(state === 'stopping' && !requiresManualReset) ? 1 : 0.3,
                  '&:hover': {
                    backgroundColor: config.color,
                    opacity: config.enabled && !(state === 'stopping' && !requiresManualReset) ? 0.8 : 0.3,
                  },
                  '&:disabled': {
                    backgroundColor: config.color,
                    opacity: 0.3,
              ...
              <Typography
                variant="body1"
                sx={{
                  color: config.enabled && !(state === 'stopping' && !requiresManualReset) ? config.color : theme.palette.text.disabled,
                  textAlign: 'center',
                  opacity: config.enabled && !(state === 'stopping' && !requiresManualReset) ? 1 : 0.3,
                }}
              >
